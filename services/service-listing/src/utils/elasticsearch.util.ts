...
import { ELASTIC_SEARCH_URL } from "@service-listing/constants/env.constants";
...
export async function checkConnection(): Promise<void> {
  ...
  while (!isConnected && retryCount < maxRetries) {
    try {
      ...
      log.info(
        `Service Hub Service Listing Elasticsearch Health Status - ${health.status}`,
      );
      isConnected = true;
    } catch (error) {
      retryCount++;
      ...
      log.log(
        "error",
        "Service Hub Service Listing checkConnection() function:",
        error,
      );
