...
import { ELASTIC_SEARCH_URL } from "@order/constants/env.constants";
...
export async function checkConnection(): Promise<void> {
  ...
  while (!isConnected && retryCount < maxRetries) {
    try {
      ...
      log.info(
        `Service Hub Order Service Elasticsearch Health Status - ${health.status}`,
      );
      isConnected = true;
    } catch (error) {
      retryCount++;
      ...
      log.log(
        "error",
        "Service Hub Order Service checkConnection() function:",
        error,
      );
