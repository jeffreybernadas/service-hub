...
import {
  ...
  PORT,
  SERVICE_NAME,
} from "@order/constants/env.constants";
import { API_PREFIX, API_VERSION } from "@order/constants/version.constant";
import healthCheckRouter from "@order/routes/health.route";
import { log } from "@order/utils/logger.util";
import enhancedResponse from "@order/middleware/enhancedResponse.middleware";
import {
  errorHandler,
  IAuthPayload,
...
import { checkConnection } from "@order/utils/elasticsearch.util";
import { initializeApm } from "@order/utils/apm.util";
import { AmqpChannel, createConnection } from "@order/config/rabbitmq.config";
...
const startServer = async () => {
  ...
  app.listen(PORT, () => {
    log.info(`Order Service (${API_VERSION}) is running on port ${PORT}.`);
  });
};
