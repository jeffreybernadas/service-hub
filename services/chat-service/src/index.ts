...
import {
  ...
  PORT,
  SERVICE_NAME,
} from "@chat/constants/env.constants";
import { API_PREFIX, API_VERSION } from "@chat/constants/version.constant";
import healthCheckRouter from "@chat/routes/health.route";
import { log } from "@chat/utils/logger.util";
import enhancedResponse from "@chat/middleware/enhancedResponse.middleware";
import {
  errorHandler,
  IAuthPayload,
...
import { checkConnection } from "@chat/utils/elasticsearch.util";
import { initializeApm } from "@chat/utils/apm.util";
import { AmqpChannel, createConnection } from "@chat/config/rabbitmq.config";
...
const startServer = async () => {
  ...
  app.listen(PORT, () => {
    log.info(`Chat Service (${API_VERSION}) is running on port ${PORT}.`);
  });
};
