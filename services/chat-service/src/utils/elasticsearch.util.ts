...
import { ELASTIC_SEARCH_URL } from "@chat/constants/env.constants";
...
export async function checkConnection(): Promise<void> {
  ...
  while (!isConnected && retryCount < maxRetries) {
    try {
      ...
      log.info(
        `Service Hub Chat Service Elasticsearch Health Status - ${health.status}`,
      );
      isConnected = true;
    } catch (error) {
      retryCount++;
      ...
      log.log(
        "error",
        "Service Hub Chat Service checkConnection() function:",
        error,
      );
