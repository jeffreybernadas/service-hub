...
import {
  ...
  PORT,
  SERVICE_NAME,
} from "@user/constants/env.constants";
import { API_PREFIX, API_VERSION } from "@user/constants/version.constant";
import healthCheckRouter from "@user/routes/health.route";
import { log } from "@user/utils/logger.util";
import enhancedResponse from "@user/middleware/enhancedResponse.middleware";
import {
  errorHandler,
  IAuthPayload,
...
import { checkConnection } from "@user/utils/elasticsearch.util";
import { initializeApm } from "@user/utils/apm.util";
import { AmqpChannel, createConnection } from "@user/config/rabbitmq.config";
...
const startServer = async () => {
  ...
  app.listen(PORT, () => {
    log.info(`User Service (${API_VERSION}) is running on port ${PORT}.`);
  });
};
