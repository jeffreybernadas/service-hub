...
import { ELASTIC_SEARCH_URL } from "@user/constants/env.constants";
...
export async function checkConnection(): Promise<void> {
  ...
  while (!isConnected && retryCount < maxRetries) {
    try {
      ...
      log.info(
        `Service Hub User Service Elasticsearch Health Status - ${health.status}`,
      );
      isConnected = true;
    } catch (error) {
      retryCount++;
      ...
      log.log(
        "error",
        "Service Hub User Service checkConnection() function:",
        error,
      );
